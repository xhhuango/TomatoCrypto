import XCTest
@testable import TomatoCrypto
import BigInt

class SignatureTests: XCTestCase {
    override func setUp() {
        super.setUp()
    }
    
    override func tearDown() {
        super.tearDown()
    }

    func testSig() {
        let mStr = ""
        let eStr = ""
        let dStr = ""
        let saltStr = "6f2841166a64471d4f0b8ed0dbb7db32161da13b"
        let msgStr = ""
        let sigStr = ""

        let m = BigUInt(mStr, radix: 16)!
        let d = BigUInt(dStr, radix: 16)!
        let e = BigUInt(eStr, radix: 16)!
        let msg = hexToBytes(hex: msgStr)
        let sig = hexToBytes(hex: sigStr)
        let privateKey = RsaPrivateKeyParameter(modulus: m, d: d)
        let publicKey = RsaPublicKeyParameter(modulus: m, e: e)
        let salt = SaltParameter(salt: hexToBytes(hex: saltStr))

        let cipher = RsaEngine()
        let hash = MessageDigest(engine: Sha1Engine())
        let mgfHash = MessageDigest(engine: Sha1Engine())
        let engine = RsaPssEngine(cipher: cipher, hash: hash, mgfHash: mgfHash)
        let signature = Signature(engine: engine)

        do {
            try signature.initialize(isSigning: true, parameters: [privateKey, salt])
            signature.update(input: msg)
            let signed = try signature.sign()
            XCTAssertEqual(signed, sig)

            try signature.initialize(isSigning: false, parameters: [publicKey, salt])
            signature.update(input: msg)
            let verified = try signature.verify(signature: signed)
            XCTAssertTrue(verified)
        } catch let error {
            XCTFail("\(error)")
        }
    }
}
