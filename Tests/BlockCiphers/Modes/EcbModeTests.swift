...
class EcbModeTests: XCTestCase {
    ...
    func testAes192() {
        let key = SimpleSecretKey(bytes: hexToBytes(hex: "8e73b0f7da0e6452c810f32b809079e562f8ead2522c6b7b"))
        let plaintext = hexToBytes(hex: "6bc1bee22e409f96e93d7e117393172a" + "ae2d8a571e03ac9c9eb76fac45af8e51" +
                                        "30c81c46a35ce411e5fbc1191a0a52ef" + "f69f2445df4f9b17ad2b417be66c3710")
    ...
    func testAes256() {
        let key = SimpleSecretKey(bytes: hexToBytes(hex: "603deb1015ca71be2b73aef0857d7781" + "1f352c073b6108d72d9810a30914dff4"))
        let plaintext = hexToBytes(hex: "6bc1bee22e409f96e93d7e117393172a" + "ae2d8a571e03ac9c9eb76fac45af8e51" +
                                        "30c81c46a35ce411e5fbc1191a0a52ef" + "f69f2445df4f9b17ad2b417be66c3710")
    ...
    func testDes() {
        let key = SimpleSecretKey(bytes: hexToBytes(hex: "123456789ABCDEF0"))
        ...
        do {
            ...
            try mode.initialize(processMode: .encryption, engine: engine, key: key, parameters: [])
            let encrypted = try mode.process(input: plaintext)
            XCTAssertEqual(encrypted, ciphertext)
            
            try mode.initialize(processMode: .decryption, engine: engine, key: key, parameters: [])
            let decrypted = try mode.process(input: encrypted)
            XCTAssertEqual(decrypted, plaintext)
        } catch let error {
            XCTFail("\(error)")
