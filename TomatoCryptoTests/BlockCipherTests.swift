...
class BlockCipherTests: XCTestCase {
    ...
    func testBlockCipher() {
        ...
        do {
            let cipher = BlockCipher(engine: AesEngine(), mode: EcbMode(), padding: NoPadding())
            
            try cipher.initialize(processMode: .encryption, key: key)
            let encrypted = try cipher.process(input: plaintext)
            XCTAssertEqual(encrypted, ciphertext)
            
            try cipher.initialize(processMode: .decryption, key: key)
