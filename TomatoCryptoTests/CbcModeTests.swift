...
class CbcModeTests: XCTestCase {
    ...
    func testAesCbc128() {
        ...
        do {
            ...
            try cbc.initialize(processMode: .encryption, engine: aes, parameters: [iv])
            let encrypted = try cbc.process(input: plaintext)
            XCTAssertEqual(encrypted, ciphertext)
            
            try aes.initialize(processMode: .decryption, key: key)
            try cbc.initialize(processMode: .decryption, engine: aes, parameters: [iv])
            let decrypted = try cbc.process(input: encrypted)
